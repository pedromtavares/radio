...
Provider.prototype.init = function(){
  ...
  process.addListener('uncaughtException', function (err, stack) {
    ...
    console.log('------------------------');
    if (err != 'Error: EBADF, bad file descriptor'){
      self.nextSong(true);
    }
  });
}
...
Provider.prototype.treatUrl = function(url, callback){
  ...
  var request = http.get(urlParser.parse(url, true), function(response){
  ...
  request.on('error', function() {
    console.log('Error on request to try to get a true mp3 URL.');
    self.nextSong(true);
  });
