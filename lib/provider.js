...
Provider.prototype.init = function(){
  ...
  // var what = "[STREAM]\ncodec_name=mp3\ncodec_long_name=MP3 (MPEG audio layer 3)\ndecoder_time_base=0/1\ncodec_type=audio\n  sample_rate=48000.000000\nchannels=2\nbits_per_sample=0\nindex=0\ntime_base=1/90000\nstart_time=0.000000\nduration=287.059967\nnb_frames=0\n[/STREAM]\n[FILE]\nfilename=15v5.mp3\nnb_streams=1\ndemuxer_name=mp3\ndemuxer_long_name=MPEG audio layer 2/3\ntrack=11\ntitle=Someone Like You\nauthor=ADELE\ncomment=11\nalbum=21\nyear=2011\ngenre=Pop\nstart_time=0.000000\nduration=287.059967\nsize=9185919.000000\nbit_rate=256000.000000\n[/FILE]";
  // 
  // var match = what.match(/bit_rate=\d*/);
  // bit_rate = match[0].split('=')[1];
  // console.log(bit_rate);
  // var match = what.match(/sample_rate=\d*/);
...
Provider.prototype.streamSong = function(){
  ...
  self.getProbeInfo(track, function(info) {
    if (!info){
      console.log('No probe info gathered, going to next song');
      self.nextSong(true);
      return;
    }
    ...
    if (!sample_rate || sample_rate != 44100){
      console.log('Target sample rate is not 44100, but : ' + sample_rate + ', going to next song.');
      self.nextSong(true);
      return;
    }
    if (!bit_rate){
...
Provider.prototype.getProbeInfo = function(track, callback) {
  ...
  if (self.app.settings.env && self.app.settings.env != 'production'){
    ...
    var proc = spawn('ffprobe', ['-show_format', '-show_streams', track]),
        probeData = '';
    proc.stdout.setEncoding('utf8');
    proc.on('data', function(data) { probeData += data; console.log(data);});
