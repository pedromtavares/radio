...
Radio.prototype.publish = function(message){
  ...
  message.listeners = self.numberOfListeners;
...
Radio.prototype.createHttpServer = function(){
  ...
  var server = http.createServer(function(request, response) {
    if (request.url == "/stream.mp3") {
      ...
      response.end("<div id='track'>"+self.currentTrack+"</div><div id='dj'>"+self.currentDJ+"</div>");
    } else {
