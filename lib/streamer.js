  ...
  self.stream.on("connect", function() {
    ...
    if (self.radio.currentDJ){
      ...
      self.streamListenersInterval = setInterval(function() {
        self.radio.getStreamListeners(function(){
          console.log("There are "+self.radio.streamListeners+" stream listeners and "+self.radio.listeners.length+" website listeners.");
        });
      }, 60000)
    }
...
Streamer.prototype.streamResponse = function(request, response, format){
  ...
  if (!self.radio.currentDJ || self.radio.listeners.length > self.settings.listenerLimit){
  ...
  if (self.settings.multipleDecoders){
