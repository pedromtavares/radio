...
function Streamer(settings, radio, chat, decoder, map){
...
Streamer.prototype.registerStreamListeners = function(){
  ...
  self.stream.on("connect", function() {
    ...
    //self.radio.addListener('187.64.106.223');
  });
  ...
  self.stream.on("metadata", function(metadata) {
    self.radio.newTrack(icecast.parseMetadata(metadata).StreamTitle);
...
Streamer.prototype.streamResponse = function(request, response, format){
  ...
  self.map.publish(request.connection.remoteAddress);
