...
Decoder.prototype.createStreams = function(samplingRate) {
  var self = this;
  var clients = [];
  if (!samplingRate){
    samplingRate = "44100";
  ...
  self.mp3 = self.createMp3Encoder(samplingRate);
  self.mp3.clients = clients;
  self.pcm = self.createRawStream(samplingRate);
};

Decoder.prototype.createRawStream = function(samplingRate){
  var self = this;
  if (!samplingRate){
    samplingRate = "44,1";
  }
  ...
  var pcm = spawn("lame", [
    ...
    "-s", samplingRate, // Sampling rate: 44,100
...
Decoder.prototype.createMp3Encoder = function(samplingRate){
  var self = this;
  var mp3 = {};
  if (!samplingRate){
    samplingRate = "44,1";
  ...
  mp3.encoder = spawn("lame", ["-S","-r","-s", samplingRate,"-","-"]);
...
Decoder.prototype.formatSamplingRate = function(unformatted) {
  var self = this;
  split = unformatted.split(',')
  if (split.length == 2){
    return unformatted;
  }else{
    first_two = unformatted.substr(0, 2);
    rest = unformatted.substr(2, unformatted.length);
